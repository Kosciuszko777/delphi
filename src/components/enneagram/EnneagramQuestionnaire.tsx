...
import { getShuffledItems, TOTAL_ITEMS } from '@/lib/enneagram/items';
import type { EnneagramAnswers } from '@/lib/enneagram/scoring';
import { answeredCount } from '@/lib/enneagram/scoring';
...
export function EnneagramQuestionnaire({ answers, onAnswer, onComplete }: EnneagramQuestionnaireProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <Progress value={progress} className="h-2 bg-oracle/10" />
      </div>
      ...
      <div className="flex items-center justify-between">
        <Button variant="ghost" onClick={goPrev} disabled={currentIndex === 0} className="gap-1.5">
        ...
        <div className="flex gap-2">
          {isAllAnswered && (
            <Button onClick={onComplete} className="bg-oracle text-oracle-foreground hover:bg-oracle/90 rounded-full px-6 gap-1.5">
          ...
          <Button variant="ghost" onClick={goNext} disabled={currentIndex === items.length - 1} className="gap-1.5">
            Next
