...
export default function SoulgraphPage() {
  const { wire } = useWire();
  ...
  useSeoMeta({
    title: 'My Soulgraph — Delphi',
    description: 'View your psychometric Soulgraph — your sovereign identity signature across multiple typology systems.',
  });
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="mt-3 text-muted-foreground max-w-lg mx-auto">
            {hasWire
              ? t('soulgraph.subtitle.has', { filled: String(filled), total: String(TOTAL_CHAMBERS) })
              : t('soulgraph.subtitle.empty')
            }
          </p>
        </div>
        ...
        {hasWire ? (
          <div className="space-y-8">
            ...
            {filled < TOTAL_CHAMBERS && (
              <div className="text-center pt-4">
                <p className="text-sm text-muted-foreground mb-4">
                  {t('soulgraph.chambersRemaining', { count: String(TOTAL_CHAMBERS - filled) })}
                </p>
                <Button asChild variant="outline" className="rounded-full">
                  <Link to="/assess">
                    {t('soulgraph.continueAssessments')}
            ...
            {/* Publish Soulgraph to Nostr */}
            <PublishWireFlow wire={wire} />

            {/* Shareable Soulgraph link — only show when published & logged in */}
            {user && publishedEvent && (
              <ShareableSoulgraphLink pubkey={user.pubkey} />
            )}
...
function ShareableSoulgraphLink({ pubkey }: { pubkey: string }) {
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 rounded-xl border border-border bg-card px-4 py-3">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <p className="text-sm text-muted-foreground truncate">
          Your public Soulgraph is visible at{' '}
